...
export default function ReportsPage() {
  ...
  const toggleArrayFilter = (
    key: "accountIds" | "categoryIds" | "tagIds",
    id: string,
  ) => {
    setFilters((f) => {
      const current = f[key];
  ...
  return (
    <Page title="Reports">
      ...
      {activeTab === "cashflow" && <CashFlowTab filters={filters} />}

      {activeTab === "expenses" && <ExpensesTab filters={filters} />}

      {activeTab === "income" && <IncomeTab filters={filters} />}

      {activeTab === "networth" && <NetWorthTab filters={filters} />}
    </Page>
  );
}
